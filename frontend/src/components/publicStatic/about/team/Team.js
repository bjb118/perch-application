...
const t4 = ['ikopyeva', 'sbutrus', 'vraman', 'amin', 'adesai',
...
// Biology
const t7 = ['sbalijepalli', 'agirgis', 'kmckernan', 'njasti',
...
for (var i = 0; i < 5; ++i) {
  for (var j = 0; j < 12; ++j) {
    ...
    var key = `${i}_${j}`;
    var colSplit = (j === 2 || j === 7) ? true : false;
    ...
    if (teamNum !== -1) {
      ...
      cardTable[i].push(
        <div key={key} className={placeholdCSS}></div>
      )
    }
    ...
    var key =
      keyBones.map(keyRow => {
        return (
          keyRow.map(idx => {
            if (idx === -1) {
              return (<div key={idx} className="key-item"> </div>)
            }
